...
import {towerRamp as ramp, glasswellTower as tower, towerPlatformOutline, towerRampHeight} from '../glasswellTower.ts'
import {glasswellRamps} from '../lowerGallery.ts'
import {RailingPath} from './RailingPath.ts'

export class GlasswellRailing extends RailingPath {
  constructor() {
    const inset = 0.12
    const west = glasswellRamps.find(value => value.side === 'west')!
