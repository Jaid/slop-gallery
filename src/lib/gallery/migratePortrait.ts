...
// Preserve loose frames from the first Cabin layout after moving its return route west.
const retiredCabinPassages = [
  new Passage('retired-cabin-approach', 'cabin', [[-25, -26], [-25, -2]], -5),
  new Passage('retired-cabin-landing', 'cabin', [[-25, 7.6], [-25, 9.65], [-20, 9.65]], 0),
]
const retiredCabinStairs = new StairFlight('retired-cabin', [-25, 0, 7.6], [-25, -5, -2], 2.6, 28, 0.55, 0.55, 'cabin')
const inRetiredCabinRoute = (position: Vec3) => retiredCabinPassages.some(passage => passage.contains(position)) || retiredCabinStairs.blocks.some(block => Math.abs(position[0] - block.position[0]) <= block.size[0] / 2 && Math.abs(position[2] - block.position[2]) <= block.size[2] / 2 && position[1] >= block.top - 1 && position[1] <= block.top + stairHeadroom)
...
export function migratePortrait(portrait: Record<string, unknown>) {
  ...
  if (portrait.hung === false && Array.isArray(position) && position.length === 3 && position.every(value => typeof value === 'number' && Number.isFinite(value)) && !insideGallery(position as Vec3) && inRetiredCabinRoute(position as Vec3)) {
    ...
    migratedPosition = [position[0] + lowerGallery.undertone.compactCenter[0] - lowerGallery.undertone.previousCenter[0], position[1] + lowerGallery.floorY - lowerGallery.undertone.previousFloorY, position[2] + lowerGallery.undertone.compactCenter[1] - lowerGallery.undertone.previousCenter[1]]
  }
  if (portrait.hung === true && Array.isArray(migratedPosition) && migratedPosition.length === 3 && migratedPosition.every(value => typeof value === 'number' && Number.isFinite(value))) {
    ...
    if (portrait.wallId === 'undertone-west' && Math.abs(x - (lowerGallery.undertone.compactCenter[0] - lowerGallery.undertone.compactSize[0] / 2 + 0.22)) <= 0.025) {
      migratedPosition = [lowerGallery.undertone.center[0] - lowerGallery.undertone.size[0] / 2 + 0.22, y, z]
    } else if (portrait.wallId === 'undertone-south' && Math.abs(z - (lowerGallery.undertone.compactCenter[1] + lowerGallery.undertone.compactSize[1] / 2 - 0.22)) <= 0.025) {
      migratedPosition = [x, y, lowerGallery.undertone.center[1] + lowerGallery.undertone.size[1] / 2 - 0.22]
    }
  }
  // Art on the removed plinth's old floor recovers on the public side of the fence.
  if (movedUndertone && portrait.hung === false) {
    migratedPosition = [...undertoneRecovery]
  }
  return {
    ...portrait,
