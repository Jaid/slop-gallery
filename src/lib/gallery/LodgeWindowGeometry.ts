...
export default class LodgeWindowGeometry {
  ...
  constructor() {
    const {roomX, tunnelX, z, bottom, top, width, lining, glassThickness} = lodgeWindow
    const height = top - bottom
    const y = (bottom + top) / 2
    // Follow the existing plank backing, including its sloped roof: no projecting box.
    const profile = timberProfile(corridorPassage.width / 2 - 0.09, corridorPassage.height, 0)
      .filter(point => point.x > 0)
      .map(point => point.set(point.x + lodge.approachX, point.y + lodge.floorY))
