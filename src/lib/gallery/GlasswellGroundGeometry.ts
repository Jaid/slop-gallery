...
import {glasswellTower as tower, towerArch, towerPlatformOutline, towerRamp, towerRampGradient, towerRampHalfWidth, towerRampHeight, towerRampSections} from './glasswellTower.ts'
import {glasswellPlatform, glasswellRamps, lowerGallery} from './lowerGallery.ts'
import {RampGeometry} from './RampGeometry.ts'

// One architectural solid supplies both the visible surface and collision.
export class GlasswellGroundGeometry extends BufferGeometry {
  constructor() {
    super()
    ...
    const block = ({position, size}: typeof glasswellPlatform) => brush(new BoxGeometry(...size).translate(...position))
    try {
