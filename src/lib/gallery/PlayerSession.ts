...
export function validatePlayerPose(value: unknown): PlayerPose | null {
  ...
  if (!Array.isArray(position) || position.length !== 3 || !position.every(n => typeof n === 'number' && Number.isFinite(n)) || typeof yaw !== 'number' || !Number.isFinite(yaw)) {
    return null
  }
...
export class PlayerSession {
  ...
  capture(value: PlayerPose) {
    ...
    if (!pose || Math.abs(pose.yaw - this.pose.yaw) < 1e-6 && pose.position.every((n, i) => Math.abs(n - this.pose.position[i]!) < 1e-6)) {
      return
    }
    this.pose = pose
    this.revision++
