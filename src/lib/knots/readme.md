...
- Eight items are displayed per candidate by default. `highlighted` items take priority, then newer plate numbers. The chosen items appear in ascending number order. Set candidate `displayLimit: 16` to review a larger batch.
