...
export class GrabbableBody {
  ...
  private capture(): BodyPose {
    ...
    return {position: [p.x, p.y, p.z], rotation: this.body.rotation(), type: this.body.bodyType(), collisions: this.colliders().map(collider => collider.isEnabled())}
  }

  rememberHome() {
    // Auto-generated colliders are installed after the rigid body’s mount effect.
    if (!this.home && this.body.numColliders()) this.home = this.capture()
  }

  private translate(position: Vec3) {
    this.body.setTranslation({x: position[0], y: position[1], z: position[2]}, false)
  }
  ...
  private restore(pose: BodyPose) {
    ...
    this.colliders().forEach((collider, i) => collider.setEnabled(pose.collisions[i] ?? false))
    this.stop()
    this.body.wakeUp()
    this.options.onAttachmentChange?.(pose.type === 1)
  }

  grab() {
    if (this.active || this.options.canGrab?.() === false) return false
    this.rememberHome()
  ...
  move(origin: Vec3, target: Vec3, delta: number, animate = true) {
    ...
    if (this.placement.hasRoom(position)) this.lastClear = position
  }
  ...
  release(throwing: boolean, direction: Vec3) {
    ...
    this.saved = null
    this.body.setBodyType(0, true)
    for (const collider of this.colliders()) collider.setEnabled(true)
    this.body.wakeUp()
    this.body.setLinvel({x: direction[0] * (throwing ? 10 : 0), y: throwing ? direction[1] * 10 + 2 : 0, z: direction[2] * (throwing ? 10 : 0)}, true)
    this.body.setAngvel({x: throwing ? 3 : 0, y: throwing ? 2 : 0, z: throwing ? 1 : 0}, true)
    return returned
  }
  ...
  recover() {
    ...
    if (!this.active && this.home && this.body.translation().y < -4) {
      ...
      const home = this.options.recoverAsDynamic?.() ? {...this.home, type: 0, collisions: this.home.collisions.map(() => true)} : this.home
      this.restore(home)
    }
  }
}
