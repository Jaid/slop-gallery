...
function wood() {
  ...
  for (let row = 0; row < 6; row++) {
    ...
    for (let line = 0; line < 26; line++) {
      ...
      for (let x = 0; x <= size; x += 8) {
        ...
        for (const target of [context, bump]) {
          if (x === 0) target.moveTo(x, gy + wave)
          else target.lineTo(x, gy + wave)
        }
      }
      context.stroke()
      bump.stroke()
  ...
  return {map: finish(color), bumpMap: finish(height, false)}
}
...
function carpet() {
  ...
  const textures = {map: finish(color), bumpMap: finish(height, false)}
  for (const texture of Object.values(textures)) texture.wrapS = texture.wrapT = MirroredRepeatWrapping
  return textures
}
...
export class RoomFloorTextures {
  ...
  private tile(surface: {map: DataTexture, bumpMap: DataTexture}, width: number, depth: number, metersPerTile: number) {
    for (const texture of Object.values(surface)) texture.repeat.set(width / metersPerTile, depth / metersPerTile)
  }

  dispose() {
    for (const surface of [this.wood, this.carpet]) {
      for (const texture of Object.values(surface)) texture.dispose()
    }
  }
}
