...
import {loadBlob, narrate, notify, roomAt, rooms, upload, useGallery, viewPortrait} from '#src/lib/gallery.ts'
...
export default function Collection() {
  ...
  return <>
    ...
    <div className="collection-grid"><button className="upload-card" onClick={upload}><Icon name="plus"/><strong>Add artwork</strong><span>Choose images from your device</span></button>{filtered.map((p, i) => <article className="art-card" key={p.id}>
...
function ArtworkDetail({portrait: p, back}: {back: () => void
  ...
  return <div className="art-detail">
    ...
    <div className="detail-actions">
      <button className="primary-button" onClick={() => viewPortrait(p.id)} disabled={!ready || !p.hung || p.merging}>Visit this work <Icon name="arrow" size={17}/></button>
