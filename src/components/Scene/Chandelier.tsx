...
import {MeshStandardMaterial, QuadraticBezierCurve3, Vector3} from 'three/webgpu'

export default function Chandelier() {
  const brass = useMemo(() => new MeshStandardMaterial({color: '#bd924c', metalness: 0.88, roughness: 0.2}), [])
  ...
  return <group position={[0, 4.05, 0]}>
    ...
    {Array.from({length: 8}, (_, i) => <group key={i} rotation={[0, i * Math.PI / 4, 0]}>
      ...
      <mesh castShadow position={[1.25, 0.32, 0]}><cylinderGeometry args={[0.045, 0.055, 0.24, 24]}/><meshStandardMaterial color="#e7c991" roughness={0.65}/></mesh>
      <mesh position={[1.25, 0.51, 0]} scale={[0.8, 1.5, 0.8]}><sphereGeometry args={[0.07, 24, 16]}/><meshStandardMaterial color="#fff0b6" emissive="#ffd36c" emissiveIntensity={4} toneMapped={false}/></mesh>
    </group>)}
