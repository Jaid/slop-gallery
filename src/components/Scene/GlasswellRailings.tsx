...
export default function GlasswellRailings() {
  const path = useMemo(() => new GlasswellRailing, [])
  ...
  return <RigidBody type="fixed" colliders={false}>
    <mesh name="glasswell-continuous-handrail" geometry={handrail} castShadow><meshStandardNodeMaterial color="#7c898c" metalness={0.85} roughness={0.25}/></mesh>
    ...
    {path.posts.map((post, i) => <group key={i} position={post.position}>
      ...
      <mesh name="glasswell-railing-post" castShadow><cylinderGeometry args={[path.postRadius, path.postRadius, post.height, 12]}/><meshStandardNodeMaterial color="#7c898c" metalness={0.85} roughness={0.25}/></mesh>
    </group>)}
