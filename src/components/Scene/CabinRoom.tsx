...
export default function CabinRoom({wood, stone}: {stone: Material
  wood: Texture}) {
  const [width, depth] = cabin.size
  return <group name="cabin-room" position={[cabin.center[0], cabin.floorY, cabin.center[1]]}>
    <RigidBody type="fixed" colliders={false}>
      <CuboidCollider args={[width / 2, 0.15, depth / 2]} position={[0, -0.15, 0]}/>
      <CuboidCollider args={[width / 2, 0.12, depth / 2]} position={[0, cabin.height + 0.12, 0]}/>
      <Box size={[width, 0.3, depth]} position={[0, -0.15, 0]} map={wood} color="#765036" roughness={0.86} envMapIntensity={0.2}/>
      <Box size={[width, 0.24, depth]} position={[0, cabin.height + 0.12, 0]} map={wood} color="#513a29"/>
      {[-3.5, 0, 3.5].map(z => <Box key={z} size={[width, 0.32, 0.3]} position={[0, cabin.height - 0.16, z]} map={wood} color="#302319"/>)}
    </RigidBody>
    ...
    <group name="cabin-fireplace" rotation={[0, Math.PI, 0]}>
      <RigidBody type="fixed" colliders="cuboid">
        <Box name="cabin-hearth" size={[3.8, 0.2, 1.2]} position={[0, 0.1, -4.35]} material={stone}/>
      ...
      <mesh name="cabin-embers" position={[0, 0.25, -4.2]} rotation={[-Math.PI / 2, 0, 0]}>
        <planeGeometry args={[1.9, 0.7]}/><meshStandardNodeMaterial color="#9b2609" emissive="#f85b12" emissiveIntensity={2}/>
      </mesh>
      <pointLight name="cabin-firelight" position={[0, 0.7, -3.85]} color="#ffb45b" intensity={20} distance={8}/>
    </group>
