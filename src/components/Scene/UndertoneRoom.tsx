...
import UndertoneCrater from './UndertoneCrater.tsx'
...
export default function UndertoneRoom({stone}: {stone: Texture}) {
  ...
  return <group name="undertone-room" position={[room.center[0], room.floorY, room.center[1]]}>
    <UndertoneCrater stone={stone}/>
    <RigidBody type="fixed" colliders={false}>
      <CuboidCollider args={[width / 2, 0.15, depth / 2]} position={[0, 5.9, 0]}/>
      <Box name="undertone-ceiling" position={[0, 5.78, 0]} size={[width, 0.18, depth]} color="#0b141d" envMapIntensity={0}/>
    </RigidBody>
    {[-10, -6, -2, 2, 6, 10].map(x => <Box key={x} position={[x, 5.5, 0]} size={[0.16, 0.4, depth]} color="#16252d" envMapIntensity={0}/>)}
    {undertoneWallFixtures.map(({side, lightX, lightZs}) => <group key={side}>
      ...
      {lightZs.map(z => <group key={z} name="undertone-wall-led" position={[lightX, 0, z]}>
