  ...
  return <>
    ...
    {levels.map(level => <div key={String(level.lower)} className={css.level}>
      ...
      <svg viewBox={`0 0 ${level.width} ${level.height}`} role="img" aria-label={level.lower ? 'Moonfall below the Antechamber, connected north by tunnel to the Oculus beneath the glass floor' : 'Upper gallery and U-turn stairs from the Antechamber'}>
        ...
        <Branch if={level.lower}><rect aria-label="Connecting tunnel" x={level.x(-lowerGallery.tunnel.width / 2)} y={level.y(lowerGallery.tunnel.northZ)} width={lowerGallery.tunnel.width * scale} height={(lowerGallery.tunnel.southZ - lowerGallery.tunnel.northZ) * scale} fill="#c3ccc4" stroke="#71857e"/></Branch>
        ...
        {level.rooms.filter(room => room.id !== 'corridor').map(room => <g key={room.id}>
          ...
          <Branch if={room.id === current}><circle cx={level.x(room.center[0]) - 2} cy={level.y(room.id === 'lobby' ? 0 : room.center[1]) + 20} r="3" fill="#596e42"/></Branch>
        </g>)}
        ...
        <Branch if={level.lower}><g aria-label="Corridor">
          ...
          <Branch if={current === 'corridor'}><circle cx={level.x(corridorRoom.center[0]) - 2} cy={level.y(corridorRoom.center[1]) + 20} r="3" fill="#596e42"/></Branch>
        </g></Branch>
