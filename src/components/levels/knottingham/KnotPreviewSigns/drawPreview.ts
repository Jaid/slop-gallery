...
import {knotPreviewIconMaximumHeight, knotPreviewIconMaximumWidth, knotPreviewTextureCellSize, knotPreviewTextureLayout} from 'knot-materials/KnotPreviewLayout.ts'
...
export default function drawPreview(context: CanvasRenderingContext2D, bay: KnotBay, images: ReadonlyMap<string, CanvasImageSource & {
  ...
  let finishIndex = 0
  for (const [row, rowCount] of layout.rowCounts.entries()) {
    const rowWidth = rowCount * layout.tileWidth
    const rowLeft = (layout.width - rowWidth) / 2
    for (let column = 0; column < rowCount; column++) {
      const finish = bay.finishes[finishIndex++]
      const centerX = rowLeft + layout.tileWidth * (column + 0.5)
      const centerY = layout.rowHeight * (row + 0.5)
      const iconCenterY = centerY - layout.rowHeight * iconCenterOffset
      const maxIcon = Math.min(layout.tileWidth * knotPreviewIconMaximumWidth, layout.rowHeight * knotPreviewIconMaximumHeight)
