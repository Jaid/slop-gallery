...
export default function KnotStudyMap({compact}: {compact?: boolean} = {}) {
  ...
  return <svg className={css.container} viewBox={`${knotGalleryBounds.minX - 1} ${knotGalleryBounds.northZ - 1} ${knotGallerySize[0] + 2} ${knotGallerySize[2] + 2}`} role="img" aria-label={`Knottingham: ${knotBays.length} model rows in the exhibition hall`}>
    ...
    <Branch unless={compact}>{knotBays.map(bay => <text key={bay.model} x={knotLayout.rowCenterX(bay.finishes.length)} y={bay.center[2] - 1.5} textAnchor="middle" className={css.label} fontSize="1.3" fontWeight="600">{bay.title}</text>)}</Branch>
    {knotExhibition.map(exhibit => <g key={exhibit.id}>
      ...
      <Branch unless={compact}><text x={exhibit.position[0]} y={exhibit.position[2] + 1.4} textAnchor="middle" className={css.label} fontSize="0.75">{exhibit.label}</text></Branch>
    </g>)}
