...
export default function Pot({kind, solid = false}: PotProps) {
  ...
  const contents = <group name={`pot-${kind}`} dispose={null}>
    <mesh name="ceramic-shell" geometry={geometry.shell} material={resources.shells[kind]} castShadow receiveShadow/>
    <mesh name="baked-in-soil" geometry={geometry.soil} material={resources.soil} receiveShadow/>
