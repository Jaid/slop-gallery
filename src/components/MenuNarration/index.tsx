...
export default function MenuNarration() {
  ...
  return <details className={css.container}>
    <summary><NarrationActivity instanceId={narration.instanceId} source={narration.source} status={narration.status} />{narration.title}</summary>
    <p>{narration.text}</p>
    <button className={css.textButton} type='button' onClick={stopNarration}>Stop narration</button>
  </details>
}
