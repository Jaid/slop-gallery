...
Level titles, entry directories, assets and the default selection live in `src/data/levels.ts`. The level is selected when Vite starts, not by a React prop, hostname, URL parameter or runtime setting. `GAME_LEVEL` accepts exactly `gallery` (the default) or `knottingham`; invalid values fail the build.

| Command | Output | Intended host |
| --- | --- | --- |
| `bun run build:gallery` | `dist/gallery` | `https://slop.gallery` |
| `bun run build:knottingham` | `dist/knottingham` | `https://knot.slop.gallery` |

Deploy the contents of the selected output folder at the host root. Alternatively, set `$env:GAME_LEVEL = 'knottingham'` in PowerShell and run `bun run build` to build that level into `dist`. Environment variables override dotenv files. The checked-in `.env.development` keeps the existing development session on Knottingham; `bun run dev:gallery` and `bun run dev:knottingham` select either explicitly.

The reusable `packages/vite-plugin-game-level` plugin makes Vite resolve `#level/*` to one folder under `src/levels` before bundling and replaces `import.meta.env.GAME_LEVEL` with a literal. Level-only components live under `src/components/levels/{gallery,knottingham}/*/index.tsx`; `src/levels/{gallery,knottingham}` supplies their build-time exports, navigation and initial collections. Scene content, navigation, initial collection and map entry points are level-specific; World, player, interaction infrastructure and rendering settings are shared. The opposite entry graph is absent, not merely hidden or lazily downloaded. Public assets bypass tree-shaking, so the build plugin copies museum art/audio only for Gallery, plus the shared icon for either level. Knot previews are imported source assets and follow Knottingham’s module graph. Every build emits `level-build.json` and fails if it includes the opposite level’s entry modules. Run `bun run check:levels` to build both and verify module and asset isolation.

Player checkpoints and IndexedDB databases keep their original storage namespaces (`slop-gallery` and `knot-gallery`), preserving existing saves across the level-ID rename and preventing collisions when alternating development builds on one origin. Knottingham validates saved poses against its own full hall bounds.
...
Vite runs through Bun with the native config loader and listens on all interfaces; `vite.tower.lan` is allowed for the local HTTPS proxy. `bun run build:dev` writes to `out/build/development`; `bun run build` writes the production bundle to `dist`; `bun run preview` serves that production build on loopback. Other nonproduction modes write to `out/build/<mode>`.

The shared Vite config enables React Compiler, level-specific page titles, media mixins, PostCSS normalization and Autoprefixer for Chrome 153+. Production adds Terser, advanced CSS optimization and flat named React/vendor/main chunks. Source maps remain enabled. Property reads retain their side effects, and CSS optimization preserves fonts, animation identifiers and stacking levels referenced by application code. The WebGPU Fiber alias and mode-specific Victoria telemetry environment settings apply to both modes. Production filenames are stable, so deployment must revalidate JavaScript and CSS rather than cache them as immutable.
...
Knottingham is a permanent, separately built level. Its independent hall starts at 56 × 74 m and expands for additional candidate rows and larger selections. Each candidate has one row of floating, grabbable Knots, with eight displayed by default and highlighted entries taking priority. Centers are 1 m above the floor for crouching inspection. Displayed Knots rotate continuously around the vertical axis, completing one revolution every 40 seconds; their colliders rotate with them, while thrown Knots retain normal physics. The hall reuses Vesper’s green plaster walls and matching trim with Dine’s cream-and-charcoal marble floor at the original tile scale, with Dine’s polished planar floor reflections enabled. No sculpture pillars or their collision meshes mount in this level. Both levels share the same World, renderer, graphics-quality settings, DPR, shadows and postprocessing.

Contributions from Astra, Claude, DeepSeek, Gemini, GLM, Grok, Kimi, Qwen and Sol retain their original titles and procedural materials. Only the eight examples from the newest prompt are retained from the old collection: #06 (Astra), #15 (Claude), #17, #19 and #24 (DeepSeek), #32 (Gemini), #40 (GLM) and #72 (Sol). Their original model credits and numbers are preserved. GPT-6 Astra (#73–80), Gemini 3.8 Flash (#81–88) and Claude Fable 5.1 (#89–96) occupy three additional rows extending north alongside these eight references. Their runtime copies correct signed even-power shader expressions and bridge the current Three.js cell-noise typings; original submissions remain untouched. Three-line plates display a unique number (retaining their original numbers), Knot title and full model title. All 32 plates share one instanced draw and texture atlas; the Knots share geometry and a code-generated studio environment. Internal aliases such as astra1 and grok7 remain stable. Model titles follow Mage’s private model catalog, with user corrections to Claude Sonnet 5, DeepSeek 4.1 Flash and Gemini 3.6 Flash. GPT-5.6 Sol is supplied separately because it is absent there.

Floating preview signs stand beside their corresponding candidate rows. They display freshly rendered sheets containing only the active Knots, with full model titles and exhibit ranges above them. The image captions show the stable exhibit numbers. These lightweight unlit planes require no pillars or collision meshes. Run `bun run update:knots` to regenerate candidate `overview.jxl` boards, model `icon.jxl` files and every item’s `icon.jxl` under `src/lib/knots`. Runtime materials and per-item author metadata live alongside those assets, adapted from the unmodified submissions in `private/knot_challenge`. See `src/lib/knots/readme.md` for the entry and preview workflow. The floor plan and minimap show the Knottingham hall and provide safe row navigation.

The sculpture pedestals use carved limestone with fluted faces, chamfered corners, stepped bases and crowns and narrow aged-brass collars set into dark reveals. Subtle procedural mineral bedding and grain run continuously across the stone. Their geometry and stone material are shared, and fixed collision meshes follow the carved surfaces while preserving the original footprint and display height.

The former secret room is now **The Antechamber**. Its east doorway leads down 44 illuminated stone steps to **Moonfall**, 8 m below the main gallery. The lower flight is twice as wide as the upper flight; a rounded landing smoothly joins their walls, moldings and brass handrails. Each handrail is a single closed mesh through both flights and the curve, with matching collision and no exposed join caps. The 28 × 28 m hall keeps its north tunnel and east stair connection, expanding west and south. Its center contains an 18 m-wide circular floor opening with a rough lunar impact crater: irregular slopes, smaller depressions, radial ridges and scattered angular rubble descend about 360 cm below the slate floor. The original orbital light sculpture floats above it. A closed ring of 32 bronze stanchions and sagging ropes protects the perimeter. Floor queries, dropped objects and saved player positions use the crater’s actual terrain; the ropes use mesh-accurate collision rather than invisible solid panels. The wall-mounted LEDs flank the full stair opening. Sienna and Moonfall have no benches, keeping their floors and approaches open. The perimeter walls remain available for your collection. The floor plan includes both levels and the stairs; navigation, imports, wall placement and saved collections use the lower floor’s elevation.
...
### Repository image formats

Bundled artwork in `public/art` and Knot icons/overviews use JXL masters. Production-only `vite-plugin-avif-only` emits AVIF using ImageMagick at quality 50, 4:2:0 chroma, speed 0 and 8-bit depth. Its content-addressed cache lives in `temp/vite-plugin-avif-only/cache`. Local development remains native JXL. Alpha saves and backups must match the current asset paths and layout; no collection migrations are provided.
