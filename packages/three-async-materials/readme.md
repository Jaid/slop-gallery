...
// Only now release your geometry, materials, textures, render targets, etc.
```
...
- Context identity includes the actual scene, camera, render target, output target, MRT, target face/layer and mip level. Repeated observations are deduplicated. New contexts observed during compilation are visited before activation.
...
Keep geometry, full materials, scene lighting/configuration and observed render targets valid during compilation. Do not run another queue or unrelated `compileAsync()` work concurrently on this renderer. The queue is intended for ordinary main-scene and reflection-style passes. It is not a promise to precompile every shadow override, XR, or custom rendering path.
...
The package's detached GPU fixture uses a standalone procedural physical material. It checks native asynchronous pipelines for color+normal MRT and single-color targets, continued placeholder rendering, changed output pixels, no new full-material pipeline/programs on activation, and WebGPU validation errors. The gallery fixture runs the same checks with its real Captured Tempest material and caller-owned resources.
