...
Generate and cache static voice samples from declarative import attributes.
...
OpenRouter's speech endpoint currently exposes PCM as the lossless transport used here. PCM is cached directly, WAV is wrapped locally without re-encoding, and Opus is transcoded from that PCM with `ffmpeg`/libopus before caching.
