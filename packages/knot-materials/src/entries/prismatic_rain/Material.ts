...
export default class PrismaticRainMaterial extends KnotMaterial {
...
// makes the clear knot feel full of weather, with different colors separating as the camera moves.
  ...
  const dropBody = vec2(rainLocal.x.mul(3.5), rainLocal.y.add(0.08)).length().smoothstep(0.035, 0.16).oneMinus()
  const dropTail = rainLocal.x.abs().smoothstep(0.018, 0.052).oneMinus().mul(rainLocal.y.negate().smoothstep(0.02, 0.38))
  const surfaceDrops = dropBody.max(dropTail.mul(0.66)).mul(near.mul(0.82).add(0.18))
  ...
  this.positionNode = p.add(normalLocal.mul(dropBody.mul(0.0011)))
