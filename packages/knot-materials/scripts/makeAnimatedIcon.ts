...
export default async function makeAnimatedIcon({id, output, ...preview}: AnimatedIconOptions) {
  ...
  try {
    ...
    const encoded = await encodeAnimation(directory)
...
if (import.meta.main) {
  ...
  if (values.help) {
    console.log('Usage: bun scripts/makeAnimatedIcon.ts <knot-id> [--output icon.animated.webm] [--browser-url URL] [--page-url URL]\n120 frames, 2 seconds, 360 degrees, 640x640 AV1/WebM. Requires ffmpeg.')
  } else {
