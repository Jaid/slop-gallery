...
By default the command writes `out/render/<id>` with three 2048 × 2048 angle PNGs (`angle_0.png`, `angle_45.png`, and `angle_90.png`), one 3840 × 2160 `closeup.png` at 0.5× distance and a 25° FOV, one 120-frame 512 × 512 `angles.animated.png` APNG orbit, two 2048 × 2048 camera-distance PNGs (`distance_near.png` at 0.75× and `distance_far.png` at 1.8×), and one 960-frame 1024 × 1024 `animation.webm`. With `--jxl`, the six stills and animated angle loop use `.jxl` instead; the animated JXL uses JPEG XL distance 4.
