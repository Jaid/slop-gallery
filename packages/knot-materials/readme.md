...
Knottingham displays up to eight non-archived entries per candidate by default. Selection prioritizes higher rarity, with ID as a deterministic tie-breaker. The selected row then runs from lower to higher rarity, putting its rarer entries at the far end. `?shots=N` changes the per-candidate cap; `?candidates=id,id` selects candidate rows. `archived: true` always excludes an entry from exhibition selection, but not from the catalogue or explicit prompt/icon requests.
