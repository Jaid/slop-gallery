...
The default controls include E (`interact`), Z (`zoom`) and X (`dump`). E calls `onInteract` once per press. Hold Z for **casual zoom**, which divides the perspective camera's original FOV by `casualZoomFactor` (default 2). While grounded and stationary, hold Shift (`sprint`) as well for **extended zoom**, which divides that same original FOV by `extendedZoomFactor` (default 3, not an additional multiplier). Releasing Shift returns to casual zoom. Movement or jump input immediately starts retracting extended zoom, and the still-held Shift goes straight to the motor for sprinting/dodging or a boosted jump; it does not wait for the camera animation. Airborne motion and residual horizontal movement also prevent extended zoom. Keeping Z and Shift held resumes extended zoom once the player comes to rest on the ground. Releasing Z or losing active input restores the normal FOV.
