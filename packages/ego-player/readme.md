...
Both modes use a quintic in-out tween: very slow at the start, fast through the middle, and very slow at the end. `casualZoomTransition` controls entering/leaving casual zoom (0.2 seconds by default). `extendedZoomTransition` controls transitions into or out of extended zoom, including a direct return to normal FOV (0.5 seconds by default). Set either duration to 0 for instant transitions of that kind. Interrupted transitions continue from the current FOV without snapping. These replace the former `zoomFactor` and `zoomTransition` props.
