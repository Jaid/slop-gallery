...
test('production gallery: visible WebGPU, physics, editing, imports, fusion and persistence', async () => {
  ...
  try {
    ...
    expect((await snapshot(page)).portraits).toHaveLength(15)
    ...
    expect(await page.$$eval('.art-card', cards => cards.length)).toBe(15)
    ...
    expect((await snapshot(page)).portraits.find(p => p.id === 'catgalaxy')!.title).toBe('A tested masterpiece')
    ...
    expect(backup.portraits).toHaveLength(15)
