...
test('production gallery: visible WebGPU, physics, editing, imports, fusion and persistence', async () => {
  ...
  try {
    ...
    expect((await snapshot(page)).portraits).toHaveLength(12)
    ...
    expect((await snapshot(page)).portraits.find(p => p.id === 'catpope')!.title).toBe('A tested masterpiece')
    ...
    await page.waitForFunction(() => (globalThis.__gallery!.snapshot as () => Snapshot)().portraits.length === 13)
    ...
    expect((await snapshot(page)).portraits).toHaveLength(13)
    ...
    expect(backup.portraits).toHaveLength(12)
