  ...
  try {
    ...
    for (const key of ['map', 'bumpMap', 'roughnessMap'] as const) {
      ...
      for (let i = 0; i < pixels.length; i += 4) {
        values.add(pixels[i]!)
        if (pixels[i + 3] !== 255) throw new Error('Clay must be opaque.')
        if (key === 'roughnessMap' && pixels[i + 1]! < 210) throw new Error('Unglazed clay must stay matte.')
      }
...
test('clay releases its color, bump and roughness maps together', () => {
  ...
  for (const key of ['map', 'bumpMap', 'roughnessMap'] as const) textures[key].addEventListener('dispose', () => disposed.push(key))
  textures.dispose()
    ...
    for (let y = 0; y < height; y += 32) {
      for (let x = 0; x < width; x += 32) {
        ...
        for (let dy = 0; dy < 32; dy++) {
          for (let dx = 0; dx < 32; dx++) total += data![((y + dy) * width + x + dx) * 4]!
        }
