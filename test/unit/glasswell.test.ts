...
import renderGlasswellRoom from '../../src/components/Scene/GlasswellRoom.tsx'
...
import {glasswellCeiling, glasswellPlatform, lowerGallery} from '../../src/lib/gallery/lowerGallery.ts'
...
await RAPIER.init()
const {floorY, glasswell, tunnel, undertone} = lowerGallery
const upper = rooms.find(room => room.id === 'daydream')!
describe('glasswell and connecting tunnel', () => {
  test('both side walls expand three meters while the centered glass and entrance stay fixed', () => {
    expect(walls.find(value => value.id === 'glasswell-west')!.center[0]).toBe(-9)
    expect(walls.find(value => value.id === 'glasswell-east')!.center[0]).toBe(9)
    expect(insideGallery([-8.9, -6.4, -20])).toBe(true)
    expect(insideGallery([8.9, -6.4, -20])).toBe(true)
    expect(insideGallery([9.01, -6.4, -20])).toBe(false)
    expect(daydream.opening.center).toEqual([0, -20])
    expect(tunnel.x).toBe(0)
    expect(glasswellCeiling.reduce((sum, slab) => sum + slab.size[0] * slab.size[1], 0)).toBe(18 * 17 - 8 * 8)
  })
  test('art follows either widened side wall exactly once without moving loose art', () => {
    for (const side of [-1, 1]) {
      const portrait = {
        ...initialPortraits[0]!,
        wallId: side === -1 ? 'glasswell-west' : 'glasswell-east',
  ...
  test('lower walls meet the ceiling underside without coplanar faces in the upper floor', () => {
    const ceilingBottom = glasswell.ceiling.topY - glasswell.ceiling.thickness
    expect(glasswell.ceiling.topY).toBe(-floorThickness)
  ...
  test('the plain cuboid fills the extended north end with a matching walkable top', () => {
    const {position, size} = glasswellPlatform
  ...
  test('the north wall moves five meters without shifting the glass or southern portal', () => {
    expect(walls.find(wall => wall.id === 'glasswell-north')!.center[2]).toBe(-31)
    expect(walls.find(wall => wall.id === 'glasswell-south')!.center[2]).toBe(-14)
    expect(glasswell.center).toEqual([0, -22.5])
    expect(glasswell.size).toEqual([18, 17])
    expect(daydream.opening.center).toEqual([0, -20])
    ...
    expect(roomAt([0, -6.4, -20])).toBe('glasswell')
    expect(roomAt([0, 1.6, -20])).toBe('daydream')
    for (const z of [-13.9, -8, 0, 4.4]) {
      expect(insideGallery([0, -6.4, z])).toBe(true)
      expect(floorHeight([0, -6.4, z])).toBe(floorY)
      expect(roomAt([0, -6.4, z])).toBe('glasswell')
    }
  ...
  test('the ceiling opening stays aligned with the transparent floor and contains no fixtures or lights', () => {
    ...
    const meshes = [
      ...
      ...glasswellCeiling.map(({center: [x, z], size: [width, depth]}) => {
        const mesh = new Mesh(new BoxGeometry(width, glasswell.ceiling.thickness, depth), material)
        mesh.position.set(x + glasswell.center[0], glasswell.ceiling.topY - glasswell.ceiling.thickness / 2, z + glasswell.center[1])
        return mesh
      }),
    ]
    try {
      ...
      for (const sprint of [false, true]) {
        for (const returning of [false, true]) {
          test(`tunnel traverses both portals at ${fps} Hz, offset ${offset}, sprint ${sprint}, returning ${returning}`, () => {
            ...
            const geometries = walls.filter(wall => ['undertone-north', 'glasswell-south'].includes(wall.id) || wall.id.startsWith('glasswell-tunnel-')).map(wall => ({
              wall,
            ...
            for (const room of [undertone, glasswell]) {
