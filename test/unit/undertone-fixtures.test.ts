...
import {undertoneWallFixtures} from '../../src/lib/gallery/undertone/fixtures.ts'
import {walls} from '../../src/lib/gallery/walls.ts'

test('every Undertone LED is backed by solid wall across its entire housing', () => {
  const room = lowerGallery.undertone
  const material = new MeshBasicMaterial
  try {
    for (const {side, lightX, lightZs} of undertoneWallFixtures) {
      const wall = walls.find(candidate => candidate.id === (side < 0 ? 'undertone-west' : 'undertone-east'))!
...
test('Amber and Undertone have no bench meshes or their automatic colliders', async () => {
  for (const room of ['AmberRoom', 'UndertoneRoom']) {
  ...
  expect(await Bun.file('src/components/Scene/CabinRoom.tsx').text()).toContain('BenchSeat')
})
