...
describe('Knot cellular fields', () => {
  ...
  test('keeps Planck Foam bubbles and spectral distance continuous across cell boundaries', async () => {
    const text = await source('planck_foam')
    expect(text).toContain('mx_worley_noise_float(q, 0.8, 0)')
    expect(text).not.toContain('cellNoiseVec3')
    expect(text).not.toContain('q.fract()')
    expect(text).toContain('p.mul(24).add(vec3(time.mul(0.2), time.mul(-0.15), time.mul(0.1)))')
    expect(text).toContain('dist.smoothstep(0.04, 0.28).oneMinus()')
    expect(text).toContain('mx_noise_float(q.mul(0.25).add(time.mul(0.6))).mul(0.5).add(0.5)')
    expect(text).toContain('spectralColor(dist.mul(12).add(time.mul(0.4)).add(view.x.mul(1.5)))')
    expect(text).toContain('this.iridescenceThicknessNode = foam.mul(450).add(120)')
    expect(text).toContain('proceduralNormal(foam, 0.0005)')
    expect(text).toContain('this.transmission = 0.96')
    expect(text).toContain('this.ior = 1.03')
    expect(text).toContain('near.mul(1.1).add(0.4)')
    expect(text).toContain('foam.pow(5)')
  })
})
