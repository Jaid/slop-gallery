...
test('Gemini speech uses PCM and the central Slop Gallery app attribution', async () => {
  ...
  expect(JSON.parse(options!.body as string)).toEqual({model: 'google/gemini-3.1-flash-tts-preview', voice: 'Algenib', input: 'GLM 5.3', response_format: 'pcm'})
...
test('all raw endpoints enforce attribution and cannot leak a key to another host', async () => {
  ...
  await generator.request('../beta/batches', {method: 'POST', headers: {'X-OpenRouter-Title': 'Wrong'}, body: '{}'})
