...
test('frequency bands measure silence, full scale and isolated speech frequencies', () => {
  ...
  frequencies.fill(0)
  for (let bin = Math.ceil(500 / (48_000 / 2048)); bin < Math.ceil(2000 / (48_000 / 2048)); bin++) frequencies[bin] = 255
...
test('meter routes narration once, clears silence and protects newer connections', () => {
  ...
  const context = {
    ...
    sampleRate: 48_000,
    destination: {},
    createMediaElementSource: () => ({connect: (node: unknown) => routes.push(node), disconnect: () => {disconnected++}}),
    createAnalyser: () => ({
      context,
      fftSize: 0,
  ...
  const audio = {paused: false, ended: false, muted: false, volume: 0.85}
  ...
  expect(routes[0]).toMatchObject({fftSize: 2048, smoothingTimeConstant: 0.72, minDecibels: -90, maxDecibels: -10})
